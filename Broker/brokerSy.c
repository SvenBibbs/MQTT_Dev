...
//...
	...
	for(int i=0; i < sizeof(ServSock[TCP]) / sizeof(ServSock[TCP][0]); i++) {
...
void newMCAddr(char *lastMCAddr) 
{
	...
	struct in6_addr addrBuff2;

	strcpy_s(lastMCAddr, sizeof(TOPIC_MC_LOWER_BOUNDARY), TOPIC_MC_LOWER_BOUNDARY);
	inet_pton(DEFAULT_FAMILY, lastMCAddr, &addrBuff);
	
	
	addrBuff.u.Word[7] = addrBuff.u.Word[7] + ntohs(i); //erhöhe um i, Achtung network byte order!
	inet_pton(DEFAULT_FAMILY, TOPIC_MC_UPPER_BOUNDARY, &addrBuff2);
	


	inet_ntop(DEFAULT_FAMILY, &addrBuff, lastMCAddr, IP6_ADDR_LENGTH);
	//upper MC Address limit should be handled here
	if(addrBuff.u.Word[7] >= addrBuff2.u.Word[7]) {
		printf("Upper Boundry reached.. \n");
	}
	else {
...
void sendUDPMsg(struct genericMsg* annMsg, char *mcGroupAddr, char *port) {
	...
	memset(&remote_host, 0, sizeof(struct sockaddr_in6));

	sscanf(port, "%d", &port_num);
	inet_pton(DEFAULT_FAMILY, "FF01::10", (void *)&remote_host.sin6_addr);
	remote_host.sin6_port = htons(port_num);
	remote_host.sin6_family = AF_INET6;
	SOCKET sock = socket(AF_INET6, SOCK_DGRAM, 0);
	if(sendto(sock, &annMsg, sizeof(annMsg), 0, (struct sockaddr_in6 *) &remote_host, sizeof(struct sockaddr_in6)) <0)
		printf("error code: %d\n",  WSAGetLastError());
...
int initServer(char *Server, char *PublisherPort, char *SubscriberPort)
{
	...
	while (1)
	{
		...
		if (FD_ISSET(ServSock[0][0], &SockSet)) //hack: we just support one TCP listening socket we get new subscriber requests
		{
